...
//정답 후 다른 사람의 풀이
//반복이 없고 수식으로 해결하기 때문에 O(1)의 시간복잡도를 갖는다.
//Math.max(n - b, 0)는 초기 빈 병 수에서 첫 번째 교환을 위한 최소 빈 병 수 b를 뺀 값. 이 값이 음수일 경우 0으로 설정한다.
//(a - b)는 각 교환 주기의 감소폭. 각 교환에서 a개의 빈 병이 줄어들고 b개의 빈 병이 추가되므로, 전체로는 a - b만큼 빈 병이 줄어든다.
//Math.floor((n - b) / (a - b))는 총 몇 번의 교환이 가능한지를 계산한다.
//* b는 각 교환에서 받을 수 있는 콜라 병 수를 곱하여 최종 받을 수 있는 콜라 병 수를 계산한다.
