...
function solution(bandage, health, attacks) {
  let answer = health
  let [dur,heal,bonus] = bandage
  let curDur = dur

  for(let i = 1; i <= attacks[attacks.length-1][0]; i++) {

      let [curHit, curDmg] = attacks.find(v=>v[0]===i) ?? [0,0]
      ...
      if(curDur === 0) {
          answer = answer+bonus>health?health:answer+bonus
          curDur = dur
      }
  }
...
function solution(bandage, health, attacks) {
  ...
  for (let [attackTime, damage] of attacks) {
    //진행시간에 대해 모두 순회하지 않고 받은 공격에 대해 순회
    //공격 받은 시점 이전까지의 회복량을 계산하기 위해 >>> 진행시간 = 공격시간 - 현재시간 -1
    let diffTime = attackTime - currTime - 1;
    //연속 성공에 대한 회복량 >>> 진행시간*회복량
    //추가 회복 조건에 대한 회복량 >>> (진행시간/시전시간)*추가회복량
    currHealth += diffTime * bandage[1] + Math.floor(diffTime / bandage[0]) * bandage[2];
    //치유량은 최대 체력을 넘을 수 없는 조건
    if (currHealth > health) currHealth = health;
    //치유량 계산 후에 피해 차감
    currHealth -= damage;
    //체력이 0 이하로 떨어지면 사망
    if (currHealth <= 0) return -1;
    //공격 받은 시점에서 다음 공격 시간까지 반복
    currTime = attackTime;
  }
