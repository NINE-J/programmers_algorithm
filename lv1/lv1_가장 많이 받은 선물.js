...
//board를 하나만 놓고 돌리다보니 friends 내부 이름과 중복되는 키 값인 경우가 있었음
//지수처럼 별도 관리할 데이터는 처음부터 분리하는 게 깔끔할 것 같다..
function solution(friends, gifts) {
  ...
  for(let user of friends) {
      let item = {}
      for(let user_ of friends) {
          if(user !== user_) item[user_] = 0
      }
      item['index'] = 0
      item['nextReceive'] = 0
      board[user] = item
  ...
  for(let gift of gifts) {
      let [sender, receiver] = gift.split(' ')
      
      board[sender][receiver]++
      board[sender]['index']++
      board[receiver]['index']--
  }
  //최대 250 미만
  for(let sender in board) {
      for(let receiver in board[sender]) {
          if(receiver === 'index' || receiver === 'nextReceive') continue
          
          let stor = board[sender][receiver]
          let rtos = board[receiver][sender]

          if(stor > rtos) board[sender]['nextReceive']++
          if(stor === rtos) {
              if(board[sender]['index'] > board[receiver]['index']) board[sender]['nextReceive']++
          }
          answer = Math.max(answer, board[sender]['nextReceive'], board[receiver]['nextReceive'])
      }
  }
...
//정답 후 다른 사람의 풀이
//인덱스로 찾을 수 있게 구성됐고 마지막에 points 배열에서 가장 많이 받는 선물만 리턴
function solution(friends, gifts) {
