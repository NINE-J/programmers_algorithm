...
function solution(rank, attendance) {
  let tmp = rank.filter((el,i)=>attendance[i]).sort((a,b)=>a-b).slice(0,3)
  let getOne = (r) => rank.indexOf(r)
  
  return 10000*getOne(tmp[0])+100*getOne(tmp[1])+getOne(tmp[2])
}

//정답 후 다른 사람의 풀이
//순위와 인덱스를 가진 배열로 가공해서 사용
function solution(rank, attendance) {
  const [a, b, c] = rank
    .map((r, i) => [r, i])
